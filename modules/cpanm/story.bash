...
if test -z $install_base; then
  ...
  if test -z $user; then
    ...
    su --shell `which bash` --login -c "cpanm $story_var $package -q && echo install" $user
  fi
  ...
  if test -z $user; then
    ...
    su --shell `which bash` --login -c "cpanm $story_var $package -q  -l $install_base && echo install" $user
  fi
